...
<template>
    <modal id="lpPickerContainer" :shown="shown" @hide="shown = false">
        <canvas id="colorPicker" width="256" height="256" />
    </modal>
</template>
...
export default {
    ...
    methods: {
        ...
        positionPicker(evt) {
            ...
            const container = document.getElementById('lpPickerContainer');
        ...
        bindHandlers(imageObj) {
            const canvas = document.getElementById('colorPicker');
            const context = canvas.getContext('2d');
            context.drawImage(imageObj, 0, 0);
            const rect = canvas.getBoundingClientRect();
            const imageData = context.getImageData(0, 0, imageObj.width, imageObj.width);
            ...
            canvas.addEventListener('mousemove', (evt) => {
                ...
                if (mouseDown && x > 0 && x < 0 + imageObj.width && y > 0 && y < 0 + imageObj.height) {
                    const r = data[((imageObj.width * y) + x) * 4];
                    const g = data[((imageObj.width * y) + x) * 4 + 1];
                    const b = data[((imageObj.width * y) + x) * 4 + 2];
                    this.callback({ r, g, b });
                }
            }, false);
        },

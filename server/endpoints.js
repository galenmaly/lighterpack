...
async function register(req, res) {
    ...
    try {
        let conflictingUsers = await knex('user')
            .where({ username })
            .orWhere({ email })
            .select('*');
        
        if (conflictingUsers.length) {
            if (users[0].username === username || (users.length > 0 && users[0].username === username)) { //hacky
            ...
            } else if (users[0].email === email || (users.length > 0 && users[0].email === email)) { //hacky
                ...
                logWithRequest(req, { message: 'User creation failed for unknown reason', users });
        ...
        try {
            ...
            const out = { username, library: JSON.stringify(newUser.library), syncToken: newSyncToken };
...
async function saveLibrary(req, res, user) {
    if (typeof req.body.syncToken === 'undefined') { // TODO: is this safe to delete?
    ...
    user.syncToken++;
    let newSyncToken = user.syncToken++;
...
async function forgotPassword(req, res) {
    ...
    try {
        ...
        try {
            await knex('user').where({username}).update({
...
async function account(req, res, user) {
    ...
    try {
        const user = await verifyPassword(user.username, String(req.body.currentPassword));
...
async function deleteAccount(req, res, user) {
    ...
    try {
        const user = await verifyPassword(user.username, String(req.body.password));
        ...
        await knex('user').where({username: user.username}).del();

...
import { getCategoryNames } from './test-helpers';
...
test.describe('CSV Export', () => {
  ...
  test('should download CSV file with the list content', async ({ page }) => {
    await page.getByText('Share', { exact: true }).hover();
    ...
    // The CSV body must contain the exported list, not just a valid header
    const body = await response.text();
    const lines = body.trim().split('\n');
    expect(lines[0]).toBe('Item Name,Category,desc,qty,weight,unit,url,price,worn,consumable');
    expect(lines).toContain('Backpack,Gear,Ultralight 40L,1,24,ounce,,0,,');
  });
});
...
test.describe('CSV Import', () => {
  ...
  test('should import quantities and units correctly', async ({ page }) => {
    ...
    // Units must survive the import: the lb item keeps lb and its weight value
    const unitValues = await itemRows.evaluateAll((nodes) =>
      nodes.map((node) => {
        const display = node.querySelector('[data-testid="unit-select"] .lpDisplay');
        return display ? (display.textContent || '').trim() : '';
      }),
    );
    expect(unitValues).toContain('lb');
    expect(unitValues).toContain('oz');

    const weightValues = await itemRows.evaluateAll((nodes) =>
      nodes.map((node) => {
        const input = node.querySelector('input.lpWeight');
        return input ? input.value : '';
      }),
    );
    expect(weightValues).toContain('0.5');
    expect(weightValues).toContain('16');
  });
});
